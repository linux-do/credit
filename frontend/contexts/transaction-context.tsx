...
interface TransactionContextState {
  ...
  updateOrderStatus: (
    orderId: string,
    updates: Partial<Pick<Order, "status" | "dispute_id">>
  ) => void;
}
...
export function TransactionProvider({
  ...
  const [lastParams, setLastParams] =
    useState<Partial<TransactionQueryParams>>(defaultParams);

  /** 使用 useRef 存储缓存 */
  const cacheRef = useRef<
    Record<string, { data: Order[]; total: number; timestamp: number }>
  >({});
  ...
  const addToCache = useCallback(
    (
      key: string,
      value: { data: Order[]; total: number; timestamp: number }
    ) => {
      const cacheKeys = Object.keys(cacheRef.current);

      if (cacheKeys.length >= MAX_CACHE_SIZE) {
        const oldestKey = cacheKeys.reduce((oldest, current) => {
          return cacheRef.current[current].timestamp <
            cacheRef.current[oldest].timestamp
            ? current
            : oldest;
        });
  ...
  const fetchTransactions = useCallback(
    async (params: Partial<TransactionQueryParams>) => {
      ...
      const cacheKey = generateTransactionCacheKey(
        queryParams as Parameters<typeof generateTransactionCacheKey>[0]
      );
  ...
  const refresh = useCallback(async () => {
    const cacheKey = generateTransactionCacheKey({
      ...
      page: currentPage,
      page_size: pageSize,
    } as Parameters<typeof generateTransactionCacheKey>[0]);
