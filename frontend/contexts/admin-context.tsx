...
export interface AdminContextState {
  ...
  updateUserPayConfig: (
    id: string,
    data: UpdateUserPayConfigRequest
  ) => Promise<void>;
...
export function AdminProvider({ children }: { children: React.ReactNode }) {
  ...
  const [userPayConfigsError, setUserPayConfigsError] = useState<Error | null>(
    null
  );
  ...
  const refetchUserPayConfigs = useCallback(async () => {
    ...
    try {
      ...
      const errorObject = handleContextError(error, "加载积分配置失败", {
        logError: true,
      });
  ...
  /** 更新用户积分配置 */
  const updateUserPayConfig = useCallback(
    async (id: string, data: UpdateUserPayConfigRequest) => {
      try {
  ...
  /** 删除用户积分配置 */
  const deleteUserPayConfig = useCallback(
    async (id: string) => {
