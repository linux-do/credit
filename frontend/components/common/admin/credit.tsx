...
function PayConfigDetailPanel({
  ...
  onEditDataChange: (
    field: keyof UserPayConfig,
    value: UserPayConfig[keyof UserPayConfig]
  ) => void;
  onSave: () => void;
  saving: boolean;
}) {
              ...
              <Input
                ...
                value={
                  editData.min_score !== undefined
                    ? editData.min_score.toString()
                    : config?.min_score?.toString() || ""
                }
                placeholder={
                  editData.min_score === undefined && !config?.min_score
                    ? "必需"
                    : ""
                }
                onChange={(e) => {
              ...
              <Input
                ...
                value={
                  editData.max_score !== undefined
                    ? editData.max_score?.toString() || ""
                    : config?.max_score?.toString() || ""
                }
                placeholder={
                  (editData.max_score === null ||
                    editData.max_score === undefined) &&
                  (config?.max_score === null ||
              ...
              <Input
                ...
                value={
                  editData.fee_rate !== undefined
                    ? (
                        parseFloat(editData.fee_rate.toString()) * 100
                      ).toString()
                    : config?.fee_rate
                      ? (
                          parseFloat(config.fee_rate.toString()) * 100
                        ).toString()
                      : ""
                }
                placeholder={
                  editData.fee_rate === undefined && !config?.fee_rate
                    ? "必需"
                    : ""
                }
                onChange={(e) => {
              ...
              <Input
                ...
                value={
                  editData.score_rate !== undefined
                    ? (
                        parseFloat(editData.score_rate.toString()) * 100
                      ).toString()
                    : config?.score_rate
                      ? (
                          parseFloat(config.score_rate.toString()) * 100
                        ).toString()
                      : ""
                }
                placeholder={
                  editData.score_rate === undefined && !config?.score_rate
                    ? "必需"
                    : ""
                }
                onChange={(e) => {
          ...
          <div className="px-3 py-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <Input
                ...
                min="0"
                value={
                  editData.daily_limit !== undefined
                    ? editData.daily_limit?.toString() || ""
                    : config?.daily_limit?.toString() || ""
                }
                placeholder={
                  (editData.daily_limit === null ||
                    editData.daily_limit === undefined) &&
                  (config?.daily_limit === null ||
                    config?.daily_limit === undefined)
                    ? "无限制"
                    : ""
                }
                onChange={(e) => {
...
export function UserPayConfigs() {
  ...
  const handleSave = async (
    ...
    await updateUserPayConfig(config.id, {
      ...
      score_rate:
        editData.score_rate?.toString() ?? config.score_rate.toString(),
      distribute_rate:
        editData.distribute_rate?.toString() ??
        config.distribute_rate.toString(),
    });
    ...
    <ManagePage<UserPayConfig>
      ...
      columns={[
        ...
        },
        {
          ...
          cell: (item) =>
            item.daily_limit
              ? `LDC ${item.daily_limit.toLocaleString()}`
              : "无限制",
          width: "min-w-[200px]",
          align: "left",
        },
        {
          header: "费率",
          cell: (item) => `${(Number(item.fee_rate) * 100).toFixed(2)}%`,
          width: "min-w-[200px]",
          align: "left",
        },
        {
          header: "分数转化率",
          cell: (item) => `${(Number(item.score_rate) * 100).toFixed(2)}%`,
          width: "min-w-[200px]",
          align: "left",
        },
        {
