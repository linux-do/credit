    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          ...
          className="bg-transparent border-0 shadow-none p-0 w-100"
        >
          <VisuallyHidden>
            <DialogHeader>
          ...
          <div className="p-5 space-y-5 bg-card border border-border/50 rounded-lg shadow-xl">
            <div className="flex flex-col items-center space-y-2 pb-4 border-b-2 border-dashed border-border/50">
              <h3 className="font-bold text-2xl tracking-wider uppercase">
                {order.app_name || "RECEIPT"}
              </h3>
            ...
            <div className="space-y-0 text-sm border-y-2 border-dashed border-border/50 py-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-dashed border-border/30 last:border-0">
                ...
                <span className="text-xs font-medium">
                  {typeConfig[order.type]?.label || order.type}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-dashed border-border/30 last:border-0">
                <span className="text-muted-foreground text-xs uppercase font-medium">
...
export function CreateDisputeDialog({
  ...
  const handleButtonClick = async () => {
    ...
    const expiryTime = new Date(
      tradeTime.getTime() +
        publicConfig.dispute_time_window_hours * 60 * 60 * 1000
    );
    const now = new Date();
  ...
  const handleCreateDispute = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "发起争议失败";
 ...
 * 取消争议弹窗
 */
export function CancelDisputeDialog({
  order,
  onSuccess,
}: {
  order: Order;
  ...
  const handleCancelDispute = async () => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "取消争议失败";
...
export function ViewDisputeHistoryDialog({ order }: { order: Order }) {
  ...
  const fetchDisputeHistory = async () => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "获取争议记录失败";
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm text-muted-foreground bg-destructive/5 border border-destructive/10 p-3 rounded-md">
                    {parseDisputeReason(disputeHistory.reason).merchantReason ||
                      "未提供拒绝理由"}
                  </div>
                </div>
              </div>
...
export function RefundReviewDialog({
  ...
  const fetchDisputeInfo = async () => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "获取争议详情失败";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "处理争议失败";
