...
import {
  typeConfig,
  statusConfig,
} from "@/components/common/general/table-filter";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
        ...
        <table className="w-full caption-bottom text-sm min-w-full">
          <TableHeader className="sticky top-0 z-30 bg-background">
            <TableRow className="border-b border-dashed hover:bg-transparent">
              ...
              <TableHead className="whitespace-nowrap text-center min-w-[50px]">
                类型
              </TableHead>
              <TableHead className="whitespace-nowrap text-center min-w-[50px]">
                状态
              </TableHead>
              <TableHead className="whitespace-nowrap text-center min-w-[80px]">
                积分动向
              </TableHead>
              <TableHead className="whitespace-nowrap text-center min-w-[80px]">
                应用名
              </TableHead>
              <TableHead className="whitespace-nowrap text-left min-w-[120px]">
                编号
              </TableHead>
              <TableHead className="whitespace-nowrap text-left min-w-[120px]">
                业务单号
              </TableHead>
              <TableHead className="whitespace-nowrap text-left w-[120px]">
                创建时间
              </TableHead>
              <TableHead className="whitespace-nowrap text-left w-[120px]">
                交易时间
              ...
              <TableHead className="whitespace-nowrap text-left min-w-[100px]">
                备注
              </TableHead>
              <TableHead className="sticky right-0 whitespace-nowrap text-center bg-background shadow-[-4px_0_8px_-2px_rgba(0,0,0,0.1)] w-[150px] z-40">
                操作
              </TableHead>
            </TableRow>
          </TableHeader>
 ...
 * 交易表格行组件
 */
const TransactionTableRow = React.memo(
  React.forwardRef<
    HTMLTableRowElement,
    {
      order: Order;
  ...
  >(function TransactionTableRow({ order, style, ...props }, ref) {
    ...
    return (
      ...
      >
        ...
        <TableCell className="text-[11px] font-medium text-left py-1">
          {order.status === "success" || order.status === "refund"
            ? formatDateTime(order.trade_time)
            : "-"}
        </TableCell>
        <TableCell className="text-[11px] font-medium text-left py-1">
        ...
        <TableCell
          className="text-[11px] font-medium text-left py-1 max-w-[250px] truncate"
          title={order.remark || ""}
        >
          {order.remark || "-"}
        </TableCell>
      ...
      `}
        >
          ...
          {(order.type === "payment" || order.type === "online") &&
            order.status === "success" && <CreateDisputeDialog order={order} />}
...
export const TransactionTableList = React.memo(function TransactionTableList({
  ...
  if (loading && transactions.length === 0) {
    return <LoadingStateWithBorder icon={ListRestart} />;
  }
